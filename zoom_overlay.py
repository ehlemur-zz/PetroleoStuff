...
from chaco.api import AbstractOverlay, BaseXYPlot, DataLabel
...
class ZoomOverlay(AbstractOverlay):
    ...
    def _selection_update_handler(self, value):
        if value is not None and self.destination is not None:
            ...
            i_low = numpy.searchsorted(self.destination.index.get_data(), r.low)
            i_high = numpy.searchsorted(self.destination.index.get_data(), r.high)
            i_mid = (i_low + i_high) / 2

            i_max = i_low + numpy.argmax(self.source.value.get_data()[i_low:i_high])

            x_max = self.source.index.get_data()[i_max]
            y_max = self.source.value.get_data()[i_max]
 
            label = self.destination.overlays[-1]
            label.data_point = (x_max, y_max)
            
            if i_max < i_mid:
                label.label_position="bottom right"
            else:
                label.label_position="bottom left"

            label.request_redraw()
    
            print(i_low + numpy.argmax(self.source.value.get_data()[i_low:i_high]))
            print(self.source.value.get_data())

            
            print(dir(self.destination.overlays[-1]), self.destination.overlays[-1])
            """
